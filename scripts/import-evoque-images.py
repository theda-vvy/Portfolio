"""Prepare the supplied Evoque exports for responsive web display."""
from pathlib import Path
import json
from PIL import Image, ImageOps
ROOT=Path(__file__).resolve().parents[1]
SOURCE=Path('/Users/thedavvy/Desktop/Desktop/Personal Work/Evoque')
OUT=ROOT/'public/projects/evoque/updated'
...
for source in sorted(p for p in SOURCE.iterdir() if p.suffix.lower() in ('.jpg', '.jpeg', '.png')):
 ...
 records[name]={'src':f'/projects/evoque/updated/{name}.webp','width':im.width,'height':im.height,'source':source.name}
(ROOT/'lib/project-assets/evoque-updated.json').write_text(json.dumps(records,indent=2)+'\n')
