...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main id="main" className="case-page">
    ...
    <div className="case-hero"><ProjectCover project={project} large /></div>
    <section className="case-overview">
      <dl className="project-details">
        <div><dt>Project</dt><dd>{project.title}</dd></div>
        <div><dt>My role</dt><dd>{project.role}</dd></div>
        <div><dt>Scope</dt><dd>{project.disciplines.join(' · ')}</dd></div>
        <div><dt>Collaboration</dt><dd>Boxless Studio</dd></div>
    ...
    <section className="identity-section">
      <div className="case-chapter-heading"><p className="eyebrow">03 / The identity</p><div><h2>{project.identityTitle}</h2><p className="creative-idea">{story.creativeIdea}</p></div></div>
      <ProjectFigure media={project.identity} className="identity-figure" />
      {project.chapters.map(chapter => <section className="case-chapter" key={chapter.label}>
